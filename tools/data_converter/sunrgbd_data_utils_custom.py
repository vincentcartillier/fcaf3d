...
class SUNRGBDData(object):
    ...
    def __init__(self, root_path, split='train', use_v1=False):
        ...
        self.feat_dir = osp.join(self.split_dir, 'mask_RCNN_features')
        self.mask_dir = osp.join(self.split_dir, 'ego_mask')
        if use_v1:
    ...
    def get_features(self, idx):
        feat_filename = osp.join(self.feat_dir, 'features', f'{idx:06d}.pkl')
        features = torch.load(feat_filename)
        features = features['p6']
        return features

    def get_egocentric_mask(self, idx):
        mask_filename = osp.join(self.mask_dir, f'{idx:06d}.mat')
        mask = sio.loadmat(mask_filename)['instance']
        return mask
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            features = np.reshape(features, (-1, 256), order="F")
            
            mask = self.get_egocentric_mask(sample_idx)
            mask = mask[:,0]
            mask = mask.astype(np.bool)

            features = features[mask,:]
            
            pc_upright_depth = np.hstack((pc_upright_depth, 
                                          features))
            ...
            info = dict()
            pc_info = {'num_features': 6+256, 
                       'lidar_idx': sample_idx}
            info['point_cloud'] = pc_info

            mmcv.mkdir_or_exist(osp.join(self.root_dir, 'points_wfeat'))
            pc_upright_depth_subsampled.tofile(
                osp.join(self.root_dir, 'points_wfeat', f'{sample_idx:06d}.bin'))

            info['pts_path'] = osp.join('points_wfeat', f'{sample_idx:06d}.bin')
